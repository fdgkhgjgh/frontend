...
// *** deleteComment Function (Moved to top level) ***
async function deleteComment(commentId, commentItem) {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/comments/${commentId}`, {  // *** CHECK YOUR ENDPOINT ***
            method: 'DELETE',
            headers: {
        ...
        if (response.ok) {
            ...
            console.error("Failed to delete comment:", response.status, data.message || '');
    ...
    addCommentForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                loadPostDetails(postId) // Refresh post details and comments
            } else {
    ...
    comments.forEach(comment => {
        ...
        if (currentUserId && currentUserId === comment.author?._id.toString()) {
            ...
            deleteButton.addEventListener('click', () => {
                deleteComment(comment._id, commentItem); // Pass commentId
            });
...
// --- Initialize: Load post details on page load ---
...
if (postId) {
    ...
    postDetailsContainer.innerHTML = '<p>No post ID specified.</p>';
}
